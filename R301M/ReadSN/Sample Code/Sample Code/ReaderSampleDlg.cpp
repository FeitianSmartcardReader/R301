...
BEGIN_MESSAGE_MAP(CReaderSampleDlg, CDialog)
	...
	ON_WM_DESTROY()
	ON_WM_CLOSE()
	ON_BN_CLICKED(IDC_TRANSMIT2, OnTransmit2)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
void CReaderSampleDlg::OnTransmit2() 
{
	// TODO: Add your control notification handler code here
		DWORD dwAtrLen=0;
	DWORD	ActiveProtocol = 0;
	unsigned char sbuf[128]={0};
	unsigned char s_buf[128]={0};
	char buf[128]={0};
	...
	if( (CButton *)GetDlgItem(IDC_CONNECT)->IsWindowEnabled() )
	{
		ret = SCardConnect(ContextHandle, ReaderName[act_Name], SCARD_SHARE_DIRECT, 
			ProtocolType, &CardHandle, &ActiveProtocol);
	...
	if (CardHandle != NULL)
	{
		...
		memcpy(sbuf,"\xa5\x5a\x31\x31",4);
		DWORD rv = SCardControl(CardHandle,SCARD_CTL_CODE(3500),sbuf,4,s_buf,sizeof(s_buf),&dwAtrLen);

		

		if (rv ==SCARD_S_SUCCESS ) 
		{
			sprintf(buf ,"%s" ,"Serial Num:");
			for(int i = 0 ;i < 8 ;i++)
			{
				sprintf( &buf[sizeof("Serial Num:") - 1 + i*2 ] , "%02x", s_buf[ 4 + i]);
			}
			SetDlgItemText(IDC_IN, (const char *)buf);
		}
		else
		{
			AfxMessageBox("GetSerialNo error!");
		}
		//
	...
	{
		AfxMessageBox("Please insert the card reader!");
	}
	if( (CButton *)GetDlgItem(IDC_CONNECT)->IsWindowEnabled() )
	{
		ret = SCardDisconnect(CardHandle, SCARD_EJECT_CARD);
